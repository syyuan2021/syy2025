...
Instead of adding positional encodings, RoPE multiplies embeddings with a rotation matrix. It modify $$f_q$$, and $$f_k$$ to inject position via rotation.

$$
f_q (x_m, m) = W_Q(x_m) \times e^{i \times x_m} , \quad   f_k(x_n, n) = W_K(x_n) \times e^{i \times x_n}
$$
